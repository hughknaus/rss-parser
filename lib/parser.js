...
class Parser {
  ...
  parseItemRss(xmlItem, itemFields) {
    ...
    if (xmlItem['media:content']) {
      for (let i = 0; i < xmlItem['media:content'].length; i++) {
        let val = xmlItem['media:content']?.[i]?.$ ?? null;
        if (!item['media:content']) item['media:content'] = new Array();
        if (val) item['media:content'].push(val)
      }
    }
